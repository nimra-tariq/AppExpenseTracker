    ...
    <div className="balanceContainer">
      ...
      <form>
        ...
        <div className="form-group">
          ...
          <button type="button" className="btn btn-info" onClick={(event) => { if (document.getElementsByTagName("input")[1].value !== '') { transactionContext.dispatcher({ type: 'addIncome', value: document.getElementsByTagName("input")[1].value }); addRecordToTable(); changeColor(event) } else { toast.error('please enter an amount'); changeColor(event) } }}>Add Income</button>
          <button type="button" className="btn btn-info" onClick={(event) => { if (document.getElementsByTagName("input")[1].value !== '') { transactionContext.dispatcher({ type: 'addExpense', value: document.getElementsByTagName("input")[1].value }); addRecordToTable(); changeColor(event) } else { toast.error('please enter an amount'); changeColor(event) } }}>Add Expense</button>
        </div>
